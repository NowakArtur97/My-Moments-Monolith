...
//        UserRegistrationDTO userRegistrationDTOExpected = (UserRegistrationDTO) userTestBuilder.withProfile(userProfileDTOExpected).build(ObjectType.DTO);
//
...
//                .withProfile(userProfileExpectedAfterObjectMapping).withRoles(Set.of(roleExpected)).build(ObjectType.ENTITY);
//
//        when(modelMapper.map(userRegistrationDTOExpected, UserEntity.class)).thenReturn(userExpectedAfterObjectMapping);
//        when(bCryptPasswordEncoder.encode(userRegistrationDTOExpected.getPassword())).thenReturn(passwordEncoded);
...
//                () -> verify(modelMapper, times(1)).map(userRegistrationDTOExpected, UserEntity.class),
//                () -> verifyNoMoreInteractions(modelMapper),
//                () -> verify(bCryptPasswordEncoder, times(1)).encode(userRegistrationDTOExpected.getPassword()),
